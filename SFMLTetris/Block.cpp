...
{
	//Checks condition of the current piece being the first to be generated.
	if (nPieceCount == 0)
	{
		nPieceCount++;
...
{
	//Checks condition to confirm if nHeldBlockType has been changed from initial value of -1.
	if (nHeldBlockType < 0)
	{
		nHeldBlockType = nCurrentBlockType;
		CreateNewBlock();
	} else { //Swaps nCurrentBlockType with nHeldBlockType.
...
void Block::RenderSwaped(sf::RenderWindow& window, int nType, sf::RectangleShape &cell)
{
	...
	//Render non-zero elements of the tetrominos array to the SFML window.
	for (int y = 0; y < 4; y++) for (int x = 0; x < 4; x++)
	{
